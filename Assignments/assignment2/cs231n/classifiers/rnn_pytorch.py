...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        embed_out = word_embedding_forward(captions_in, W_embed)
        rnn_out = rnn_forward(embed_out, h0, Wx, Wh, b)
        score = temporal_affine_forward(rnn_out, W_vocab, b_vocab)
        loss = temporal_softmax_loss(score, captions_out, mask)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        ###########################################################################
        captions_list = []
        start = torch.full((N,), self._start, dtype=torch.long, device=features.device)
        # affine_forward
        h0 = affine_forward(features, W_proj, b_proj)
        # word embedding
        embed_out = W_embed[start]
        h_prev = h0
        x_now = embed_out
        for i in range(max_length):
          # RNN
          rnn_out = rnn_step_forward(x_now, h_prev, Wx, Wh, b)
          # temporal_affine_forward -> score -> indeces
          vocab_score = rnn_out @ W_vocab + b_vocab
          word_indices = torch.argmax(vocab_score, dim=1)
          h_prev = rnn_out
          # word embedding
          x_now = W_embed[word_indices]
          # store
          captions_list.append(word_indices)
        captions = torch.stack(captions_list, dim=1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
