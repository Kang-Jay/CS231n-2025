...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption_encoded = self.positional_encoding(caption_embedded)
        # image features -> memory(N, 1, W)
        memory = self.visual_projection(features).unsqueeze(1)
        # causal mask(T, T)
        tgt_mask = torch.tril(torch.ones(T, T))
        # transformer decoder (N, T, W) -> (N, T, V)
        decoder_out = self.transformer(caption_encoded, memory, tgt_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        # embedding(sequece & position)
        out = self.patch_embed(x)
        out_embedded = self.positional_encoding(out)
        # Encoder Blocks
        out_encoder = self.transformer(out_embedded)
        # Average Pooling -> feature vector(N, patch_features)
        out_mean = out_encoder.mean(dim=1)
        # affine
