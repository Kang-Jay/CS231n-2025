...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        x_start_param = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        noise_param = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        x_start = (x_t - noise_param * noise) / x_start_param
        ####################################################################
        return x_start
    ...
    def predict_noise_from_start(self, x_t, t, x_start):
        ...
        x_start_param = extract(self.sqrt_alphas_cumprod, t, x_t.shape)
        noise_param = extract(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        pred_noise = (x_t - x_start_param * x_start) / noise_param
        ####################################################################
        return pred_noise
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      and sample x_{t-1}.
        ##################################################################
        # predict noise & calculate x_0
        pred = self.model(x_t, t, model_kwargs)
        if self.objective == "pred_noise":
            x0_hat = self.predict_start_from_noise(x_t, t, pred)
        else:
            x0_hat = pred
        # clamp
        x0_hat = x0_hat.clamp(-1.0, 1.0)
        # mean & std
        posterior_mean, posterior_std = self.q_posterior(x0_hat, x_t, t)
        # sample x_{t-1}
    ...
    def q_sample(self, x_start, t, noise):
        ...
        ####################################################################
        x_start_param = extract(self.sqrt_alphas_cumprod, t, x_start.shape)
        noise_param = extract(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        x_t = x_start_param * x_start + noise_param * noise
        ####################################################################
        return x_t
