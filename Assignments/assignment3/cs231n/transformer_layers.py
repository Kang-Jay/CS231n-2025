...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # score & mask
        Attn_raw = torch.matmul(Q, K.transpose(-1, -2)) / scale 
        if mask != None:
            Attn_score = Attn_raw.masked_fill(mask == 0, value=float("-inf"))
        else:
            Attn_score = Attn_raw
        # softmax & dropout
        Attn_weight = self.attn_drop(torch.softmax(Attn_score, -1))
        # @V & concat & affine
        Attn_out_head = Attn_weight @ V
        Concat = Attn_out_head.transpose(1, 2).contiguous().view(N, S, E)
...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory, tgt_mask=None):
        ...
        Returns:
        - out: the Transformer features, of shape (N, T, D)
        """
        ...
        ############################################################################
        # Masked Self-Attn Layer (implemented)
        # Cross-Attn Layer
        shortcut = tgt
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x_mid = x.reshape(N, C, num_patch, self.patch_size, num_patch, self.patch_size).permute(0, 2, 4, 1, 3, 5)
        x_patches = x_mid.reshape(N, self.num_patches, self.patch_dim)
...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask=None):
        ...
        ############################################################################
        # Self-Attn -> Dropout -> Resisual -> LN
        out = self.self_attn(src, src, src, src_mask)
        out = self.dropout_self(out)
        out_attn = self.norm_self(src + out)
        # FFN -> Dropout -> Resisual -> LN
        out = self.ffn(out_attn)
        out = self.dropout_ffn(out)
        out = self.norm_ffn(out_attn + out)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return out
