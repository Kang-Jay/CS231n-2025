...
def sim(z_i, z_j):
    ...
    ##############################################################################
    norm_dot_product = z_i @ z_j.T / (torch.linalg.norm(z_i) * torch.linalg.norm(z_j))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        def lij(z_i, z_j, i):
            ...
            for tgt in range(2*N):
                if tgt != i:
                    lij_under += torch.exp(sim(z_i, out[tgt]) / tau)
            lij_above = torch.exp(sim(z_i, z_j) / tau)
            result = -torch.log(lij_above / lij_under)
            return result
        lk_kN = lij(z_k, z_k_N, k)
        lkN_k = lij(z_k_N, z_k, k + N)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (left * right).sum(dim=1, keepdim=True)   # (N, 1)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return pos_pairs
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    """Compute the  loss L over a batch (vectorized version). No loops are allowed.
    ...
    rows = torch.arange(2 * N, device=sim_matrix.device)
    # 0..N-1 对应 N..2N-1；N..2N-1 对应 0..N-1
    pos_cols = (rows + N) % (2 * N)
    pos_sim = sim_matrix[rows, pos_cols] 
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(pos_sim / tau) # (2N,)
